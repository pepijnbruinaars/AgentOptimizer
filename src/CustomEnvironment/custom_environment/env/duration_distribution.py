...
class DurationDistribution:
    ...
    def _generate_raw_sample(self, size: int) -> List[float]:
        ...
        def ensure_list(samples):
            ...
            return [float(x) for x in samples.tolist()] if hasattr(samples, 'tolist') else list(samples)
        ...
        if self.type == DistributionType.FIXED:
            ...
            if self.mean is None or self.min is None:
                raise ValueError("Mean and min must be provided for exponential distribution")
    ...
    def scale_distribution(self, alpha: float) -> "DurationDistribution":
        return DurationDistribution(
            name=self.type,
            mean=self.mean * alpha if self.mean is not None else None,  # Mean: scaled by multiplying by [alpha]
            var=self.var * alpha * alpha if self.var is not None else None,  # Variance: scaled by multiplying by [alpha]^2
            std=self.std * alpha if self.std is not None else None,  # STD: scaled by multiplying by [alpha]
            minimum=self.min * alpha if self.min is not None else None,  # Min: scaled by multiplying by [alpha]
            maximum=self.max * alpha if self.max is not None else None,  # Max: scaled by multiplying by [alpha]
        )
...
def get_best_fitting_distribution(
    ...
    if all(d == 0.0 for d in filtered_data):
        ...
        if not dist_candidates:
            raise ValueError("No suitable distribution candidates found")
...
def _check_fix(data: List[float], delta: float = 20.0) -> Optional[float]:
    ...
    for d1 in counter:
        if value is None or (counter[d1] > counter[value] and 
            sum(1 for d2 in data if abs(d1 - d2) < delta) / len(data) > 0.95):
