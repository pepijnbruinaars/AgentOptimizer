...
class MAPPOTrainer:
    ...
    def train(self):
        ...
        while self.timesteps_done < self.total_timesteps:
            ...
            self.agent.store_experience(obs, actions, action_probs, sum(rewards.values()), done, value)
            ...
            if done:
                ...
                if self.episodes_done % 10 == 0:
                    ...
                    print(f"Episode {self.episodes_done} | Timestep {self.timesteps_done}/{self.total_timesteps} | "
                         f"Avg. Reward: {avg_reward:.2f} | Avg. Length: {avg_length:.2f}")
            
            # Periodic evaluation
            if self.timesteps_done % self.eval_freq == 0:
                eval_reward = self.evaluate()
                print(f"Evaluation at timestep {self.timesteps_done}: {eval_reward:.2f}")
            ...
            if self.timesteps_done % self.save_freq == 0:
                self.agent.save_models(f"{self.save_dir}/checkpoint_{self.timesteps_done}")
                print(f"Checkpoint saved at timestep {self.timesteps_done}")
        
        # Save final model
        self.agent.save_models(f"{self.save_dir}/final")
        print(f"Training completed after {self.episodes_done} episodes and {self.timesteps_done} timesteps.")
